"""
Dashboard filter/sort/summary helpers (Phase 3 polish).

Kept as small, pure, framework-independent functions so they're fully unit
testable without Streamlit (which can't be installed in the assistant's own
sandbox - see PROJECT_STATUS.md known issues). `app.py` calls these with
whatever rows came back from database.py (sqlite3.Row objects support the
same `row["field"]` access dict rows do, so these work with either).
"""
...
def sum_hps(rows: Iterable) -> float:
    ...
    the FILTERED rows so the total recalculates with the active filter, per
    Nikol's request). Rows with no numeric hps_value are skipped, not
    treated as zero-but-counted."""
    total = 0.0
    for row in rows:
